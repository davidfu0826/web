...
RSpec.describe(MeetingsController) do
  ...
  describe('GET #index') do
    it('renders properly') do
      create(:meeting, kind: :council, year: '2017', title: 'FM 12', ranking: 12)
      create(:meeting, kind: :council, year: '2017', title: 'FM 9', ranking: 9)
      get(:index)
      expect(response).to have_http_status(200)
      meetings = []
      assigns(:meetings).each do |kind, ms|
        ms.each do |m|
          meetings << m.to_s
        end
      end
      expect(meetings).to eq(['2017 FM 12', '2017 FM 9'])
    end
  end
  ...
  describe('PATCH #update') do
    it('valid parameters') do
      ...
      post(:update, id: meeting.to_param, meeting: attributes)
      meeting.reload
